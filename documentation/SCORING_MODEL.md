...
| Added **Manager Quality** as a 6th pillar (10%) | v1 had no signal for fund manager skill or AMC governance â€” a critical blind spot identified in the book |
...
| **Category-aware weight tables** for equity vs. debt vs. hybrid vs. index | One weight set doesn't fit all â€” index funds should not be scored on active-manager metrics |
...
The Fund Scoring Model v2 is a **category-normalized, multi-pillar quantitative framework** that evaluates mutual funds across **six independent pillars** and produces a single composite score (0â€“100). The model is designed to reflect risk-adjusted value and manager quality, not raw past performance.
...
Final Score = 0.30 Ã— Performance

            + 0.25 Ã— Risk / Stability

            + 0.15 Ã— Cost

            + 0.15 Ã— Composition & Liquidity

            + 0.15 Ã— Manager Quality

            âˆ’ Red Flag Penalty   (max 20 pts)
...
| **Composition & Liquidity** | 15% | â€” | 15% | â€” |

| **Debt Quality** | â€” | 15% | 5%* | â€” |

| **Manager Quality** | 15% | 10% | 10% | 5% |

| **Red Flag Penalty** | âˆ’20 max | âˆ’20 max | âˆ’20 max | âˆ’15 max |



> *For hybrid funds, the Debt Quality pillar is scaled to the fund's average debt allocation (e.g., a 65/35 equity-debt fund applies Debt Quality at 35% Ã— 15% weight). The remaining weight flows to Composition.
...
| Return Consistency Score | 5% | See Â§3.1.3 | Penalizes high variance in calendar year returns |
...
score = clamp((value âˆ’ low) / (high âˆ’ low), 0, 1) Ã— 100
...
Measures how stable the fund's calendar year returns are. A fund that returns 12% every year is better than one that returns 30%, âˆ’10%, 30%, âˆ’10%, even if CAGR is similar.
...
consistency_score = clamp(1 âˆ’ std(calendar_year_returns) / 30, 0, 1) Ã— 100

```



A standard deviation of 0% in annual returns â†’ score 100. A std of 30% (very erratic) â†’ score 0.
...
For index funds, the only performance metric that matters is **tracking quality** â€” how faithfully the fund replicates its index. Active alpha generation is not expected.
...
| 1Y Tracking Difference (fund return âˆ’ index return) | 50% | `normalize(-tracking_diff_1y, low=-1.5, high=0)` |
...
- If 3Y CAGR missing â†’ substitute 5Y or 1Y, reduce weight proportionally, mark pillar `Provisional`

- If all trailing CAGRs missing â†’ pillar is `Unrated`

- If benchmark missing â†’ excess return sub-score = 0 (not penalized, but noted)
...
### 3.2 Risk / Stability Score (0â€“100)
...
| Sortino Ratio (3Y) | 25% | `normalize(sortino, low=0, high=3.0)` | Penalizes only bad volatility â€” the book emphasizes this distinction from Sharpe |
...
| Downside Capture Ratio (3Y) | 20% | `normalize(100 âˆ’ dc, low=-30, high=30)` | Did the fund fall less than the market when it fell? |
...
| Beta (3Y) | 5% | `normalize(1.2 âˆ’ beta, low=0, high=0.7)` | Lower beta preferred, but only as tiebreaker â€” a low-beta fund with low returns is not good |

| Upside/Downside Capture Asymmetry | 5% | `normalize(uc âˆ’ dc, low=-20, high=40)` | Ideally: capture more upside than downside; asymmetry > 0 is good |
...
| Downside Capture (3Y) | 20% | `normalize(100 âˆ’ dc, low=-20, high=30)` |
...
| 80â€“100 | Exceptional downside protection; fund consistently absorbs shocks well |

| 60â€“79 | Above-average risk control with minor vulnerabilities |

| 40â€“59 | Category-average volatility and drawdown |

| 20â€“39 | Notable downside risk; max drawdowns or high capture ratios |

| 0â€“19 | Severe risk profile; large drawdowns and/or negative Sharpe |
...
- If 3Y risk metrics unavailable â†’ use 5Y equivalents; mark `Provisional`

- If neither 3Y nor 5Y available â†’ pillar `Unrated`
...
### 3.3 Cost Score (0â€“100)



Measures **total cost efficiency** relative to category norms. Cost is a guaranteed drag on returns â€” unlike performance, it is entirely within the fund house's control.
...
| Category Type | Excellent (<) | Good (<) | Average (<) | Poor (â‰¥) |

|---|---|---|---|---|

| Equity â€“ Direct | 0.30% | 0.60% | 1.00% | 1.50% |

| Equity â€“ Regular | 0.80% | 1.20% | 1.75% | 2.25% |

| Index / ETF â€“ Direct | 0.05% | 0.15% | 0.30% | 0.50% |

| Index / ETF â€“ Regular | 0.15% | 0.30% | 0.50% | 0.80% |

| Debt â€“ Direct | 0.20% | 0.40% | 0.70% | 1.00% |

| Debt â€“ Regular | 0.50% | 0.80% | 1.20% | 1.50% |

| Hybrid â€“ Direct | 0.40% | 0.70% | 1.10% | 1.60% |

| Hybrid â€“ Regular | 0.90% | 1.30% | 1.80% | 2.30% |
...
er_band_score = map(er_vs_thresholds) â†’ 100 / 75 / 50 / 25 / 0
...
| â‚¹5,000â€“10,000 Cr | 85 |

| â‚¹1,000â€“5,000 Cr | 70 |
...
> **Why AUM matters for cost**: Small AUM funds spread fixed costs (compliance, operations, fund manager salary) over fewer investors, structurally raising expense ratios. Very large AUM (>â‚¹10,000 Cr) gets economies of scale. However, for small-cap funds, very large AUM constrains the fund manager's ability to deploy capital efficiently â€” this is handled via a red flag in Â§3.6, not the cost pillar.
...
### 3.4 Composition & Liquidity Score (0â€“100)
...
| Top-10 Concentration | 30% | `clamp((90 âˆ’ top10_pct) / 50, 0, 1) Ã— 100` | High concentration amplifies single-stock risk; book explicitly warns about this |

| Sector HHI (Herfindahl-Hirschman Index) | 25% | `clamp(1 âˆ’ HHI / 0.5, 0, 1) Ã— 100` | Sector concentration risk; important for thematic drift in multi-cap / flexi-cap |

| Holdings Count | 15% | `clamp((count âˆ’ 5) / 45, 0, 1) Ã— 100` (cap at 60 stocks) | Too few = concentration; too many (>60) = closet indexing |

| Portfolio Liquidity Score | 20% | See Â§3.4.2 | CRISIL's CMFR includes this; critical for small/mid-cap funds |

| Portfolio Turnover | 10% | `normalize(1 âˆ’ turnover/5, 0, 1) Ã— 100` | Very high turnover (>400%) indicates churn and higher hidden transaction costs |
...
Based on the tradability of the fund's holdings. Specifically, what percentage of the fund's portfolio can be liquidated within a reasonable timeframe (3â€“5 trading days) without significant market impact?
...
semi_liquid_pct = weight in mid-cap (rank 101â€“250)

illiquid_pct = weight in small-cap (rank 251+) + unlisted + debt instruments with low trading volume



liquidity_score = (1.0 Ã— liquid_pct + 0.6 Ã— semi_liquid_pct + 0.1 Ã— illiquid_pct) Ã— 100
...
turnover < 30%   â†’ buy-and-hold (PPFAS style) â†’ high score

turnover 30â€“80%  â†’ actively managed          â†’ medium score

turnover 80â€“200% â†’ frequent rebalancing       â†’ lower score

turnover > 400%  â†’ very high churn (Quant style) â†’ low score unless justified by IR

```



The book's Quant vs PPFAS case study shows that high turnover can coexist with high performance â€” but this must be justified by an above-average Information Ratio. If IR is strong (>0.7), the turnover penalty in Composition is offset by the IR gain in Risk. If IR is weak, high turnover is pure value destruction.
...
- If no holdings data â†’ pillar `Unrated`, note reason explicitly. Apply âˆ’2 pt red flag.

- Partial holdings (top 10 known, sector unknown) â†’ compute available sub-metrics; weight redistribute proportionally; mark `Provisional`.
...
### 3.5 Debt Quality Score (0â€“100)



*Applies to debt funds only. Also partially applied to hybrid funds (see weight table in Â§2.2).*
...
| Credit Quality Score | 35% | See Â§3.5.2 | Single largest determinant of default risk in a debt fund |

| Modified Duration Score | 25% | See Â§3.5.3 | Duration mismatch with investor holding period = interest rate risk |

| Yield-to-Maturity vs Category Average | 20% | `normalize(ytm âˆ’ cat_ytm_avg, low=-2, high=2)` | Is the yield pickup justified by credit risk, or is it free lunch? |
...
credit_score = Î£(weight_i Ã— score_i) / Î£(weight_i)

```



**Why**: SEBI's Potential Risk Matrix (PRC) classifies credit risk into Low/Medium/High. Our score mirrors this but with finer granularity. AAâˆ’ and below constitute meaningful credit risk â€” the book warns that Credit Risk Funds investing in sub-AAA paper carry issuer default risk.
...
| Overnight | < 0.1Y | âˆ’20 pts if > 0.5Y |

| Liquid | < 0.25Y | âˆ’15 pts if > 0.5Y |

| Ultra Short Duration | 0.25â€“0.5Y | âˆ’10 pts if outside Â±50% |

| Low Duration | 0.5â€“1Y | âˆ’10 pts if outside Â±50% |

| Short Duration | 1â€“3Y | âˆ’10 pts if outside Â±50% |

| Medium Duration | 3â€“4Y | âˆ’10 pts if outside Â±50% |

| Long Duration / Gilt | > 7Y | âˆ’5 pts if < 5Y (too conservative for category) |
...
duration_score = clamp((max_band âˆ’ actual_duration) / (max_band âˆ’ min_band), 0, 1) Ã— 100
...
### 3.6 Manager Quality Score (0â€“100)
...
| AMC Governance Score | 25% | See Â§3.6.2 | No amount of performance excuses front-running or regulatory violations |

| # of Funds Managed by Same Manager | 15% | `normalize(10 âˆ’ num_funds_managed, low=0, high=9)` | A manager overseeing 15 funds cannot give adequate attention to each one |

| Closet Indexing Check (RÂ²) | 15% | See Â§3.6.3 | Investors paying active management fees deserve active management |
...
- **SEBI regulatory actions**: Any SEBI order, penalty, or show-cause notice against the AMC in the last 5 years â†’ âˆ’15 pts

- **Front-running / insider trading incidents**: Confirmed cases â†’ âˆ’25 pts (effectively tanks this sub-metric)

- **AUM growth trend** (proxy for investor confidence): Consistent AUM growth over 3 years â†’ positive signal

- **Disclosure quality**: Timely monthly factsheet publication, detailed portfolio disclosures â†’ positive signal

- **No negative news / controversies**: Clean record â†’ full marks



AMC Governance Score is scored 0â€“100 as a judgement-plus-data composite and is updated quarterly.
...
Funds that charge active fees but mirror the index waste investor capital. RÂ² (R-squared) measures how much of a fund's return variance is explained by its benchmark:



```

if RÂ² > 0.95 and fund is actively managed:
...
| AMC Governance | 60% | Still relevant â€” ETF providers can still have compliance failures |
...
### 3.7 Red Flag Penalties (0â€“20 pts maximum deduction)



Red flags are **subtractive** â€” they reduce the final composite score regardless of other pillar scores. The maximum penalty cap is raised from 15 to **20 pts** in v2.
...
| Missing benchmark | âˆ’3 pts | No benchmark mapped for the category | Can't measure excess return or downside capture |

| Very high expense ratio | âˆ’5 pts | ER > 2.5% equity; > 1.5% debt | Silent wealth destroyer; the book's chapter on expense ratio |

| Very low AUM | âˆ’3 pts | AUM < â‚¹100 Cr | Limited economies of scale; liquidity risk |

| Very high AUM (small cap only) | âˆ’4 pts | AUM > â‚¹20,000 Cr AND category is Small Cap | Fund becomes too large to exploit small-cap inefficiencies |

| Extreme concentration | âˆ’5 pts | Single holding > 50% of portfolio | Massive unsystematic risk; not diversification |

| Insufficient NAV history | âˆ’8 pts | < 1 year (252 trading days) of NAV data | Cannot compute meaningful risk metrics |

| No holdings data | âˆ’2 pts | Holdings data unavailable | Cannot score Composition pillar |

| Benchmark mismatch (low RÂ²) | âˆ’3 pts | RÂ² < 50% when benchmark is available AND not intentionally unconstrained | Benchmark comparison is meaningless |

| SEBI regulatory action | âˆ’8 pts | Any active SEBI penalty/order against the AMC | Governance failure; protects investors from compromised AMCs |

| Front-running / insider trading confirmed | âˆ’12 pts | Confirmed by SEBI/court order | Severe trust violation; nearly nullifies Manager Quality pillar |

| Very high turnover without IR justification | âˆ’3 pts | Turnover > 500% AND Information Ratio < 0 | Pure churn; destroys value for investors per book's analysis |

| Credit Risk Fund with < AA avg credit | âˆ’4 pts | Debt funds with weighted avg credit < AA | High default risk; appropriate for informed investors, not general recommendation |



Maximum total penalty is capped at **âˆ’20 points** (capped at the top of the deduction stack).
...
| **Fully Rated** | ðŸŸ¢ Rated | 3Y+ NAV history; 3Y risk metrics; trailing returns (1Y, 3Y, 5Y); at least partial holdings; benchmark mapped; manager tenure available |

| **Rated (Partial)** | ðŸ”µ Rated-P | All above except missing one minor data point (e.g., no 5Y return, or no sector data) |

| **Provisional** | ðŸŸ¡ Provisional | Missing 3Y risk metrics OR no holdings data OR NAV < 3 years but > 1 year |

| **Unrated** | âšª Unrated | < 1 year NAV history, or multiple critical pillars cannot be computed (e.g., no returns AND no risk data) |
...
| 80â€“100 | ðŸŸ¢ Outstanding | Best-in-class risk-adjusted profile; strong manager quality and governance |

| 65â€“79 | ðŸ”µ Strong | Excellent fund with minor weaknesses; suitable for most investors |

| 50â€“64 | ðŸŸ£ Good | Solid fund with some tradeoffs; worthwhile with further due diligence |

| 35â€“49 | ðŸŸ¡ Fair | Average profile â€” notable concerns in one or more areas |

| 20â€“34 | ðŸŸ  Weak | Multiple weaknesses; close monitoring required before investing |

| 0â€“19 | ðŸ”´ Poor | Significant structural, governance, or performance concerns |



> The thresholds are **not percentiles** â€” they represent absolute quality bands calibrated to Indian market data. A score of 75 means the fund is genuinely strong in most dimensions, not merely that 25% of funds score higher.
...
- High RÂ² (they should be 0.99+ vs their benchmark)
...
A â‚¹50 Cr average daily traded volume scores 100; negligible volume scores 0.
...
- The AUM red flag threshold for small-cap funds is much stricter: AUM > â‚¹20,000 Cr triggers a âˆ’4 pt red flag (since a very large small-cap fund cannot invest in small caps without moving prices)
...
- Allocation drift check: if rolling 12-month average large-cap allocation > 85% for a flexi-cap fund â†’ closet indexing signal is flagged in Manager Quality
...
2. **The model uses price index benchmarks**, not TRI (Total Return Index). TRI benchmarks are typically 1â€“2% higher annually. Excess return comparisons may slightly favour the fund vs SEBI-mandated TRI comparisons. This is a known limitation of public benchmark data availability.
...
4. **The model does not personalize for investor goals, risk tolerance, or time horizon.** A high-scoring fund may still be unsuitable for a specific investor. The book's asset allocation framework (Â§5.2) should guide which fund types are appropriate.
...
7. **Category comparisons have limits.** A score of 70 in Small Cap Equity and 70 in Overnight Debt are not equivalent risk-adjusted comparisons â€” the categories have fundamentally different risk profiles.
...
Understanding why the six pillars exist together â€” and not in isolation â€” is essential for interpreting scores correctly.
...
A fund's 3Y CAGR is the most commonly cited metric, but it suffers from **period-selection bias**. A fund that performs brilliantly in 2020â€“2023 (a bull market) may look outstanding on trailing returns while having poor downside protection, high costs, and a portfolio that's been lucky rather than skilled. The book's Quant vs PPFAS case study illustrates this: Quant outperforms in rallies, PPFAS in corrections â€” neither is simply "better."
...
The Sortino Ratio, Information Ratio, and Downside Capture together form the **risk-return contract**: they tell you whether the returns you're getting are being earned efficiently. A fund earning 15% with a Sortino of 3 is fundamentally different from one earning 15% with a Sortino of 0.5 â€” the first is a skilled manager; the second is a volatility machine.
...
A debt fund's portfolio structure is fundamentally different from equities. The concentration of risk in debt is in **issuer credit quality and duration mismatch**, not sector or stock diversification. The Franklin Templeton 2020 event was not a composition failure â€” it was a credit quality and liquidity failure. The Debt Quality pillar captures this.
